...
2. Выбираем пункт "Создать новый уровень" и поподаем в редактор уровня.
...
2. Запускаем клиента на расбери, который передает пакеты данных с энцефалографа.
    ...
    (1) В директории с проектом выполнить `mkfifo fifo` => в директории появится файл fifo - именованный канал
    
    
    (2) Для работы с веб-сокетами python на Raspberry необходимо установить библиотеку Websocket:
  
    `sudo pip install websocket-client`
    

    (3) Сначала запускаем скрипт чтения из буфера `hackathon_Read-fifo-file_and_Send-ws.py`
...
if __name__ == "__main__":
  ...
  ```
   ...
   (4) Далее, запускаем `основной скрипт` записи данных с  3х каналов ЭЭГ `hackathon.py`
  
    
   (5) Результат можно видеть на графике в консоли
