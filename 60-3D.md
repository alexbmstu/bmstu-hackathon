...
##### 5.3.2.1. Заходим на url консоли по `http` (так как вебсокеты в проекте работают по незашифрованному протоколу ws) и указываем url развернутого сокет-сервера `neuro-console-ws-server`: ws://your_server_name_for_neuro-console-ws-server/
...
##### 5.3.2.2. Запускаем клиента на расбери, который передает пакеты данных с энцефалографа.
...
`unzip stage3.zip`
   ...
   D. Сначала запускаем скрипт чтения из буфера без вывода эхо на консоль `python hackathon_R.py > /dev/null 2>&1 &`
...
if __name__ == "__main__":
  ...
  ```
   ...
   D. Далее, запускаем `основной скрипт` записи данных с  3х каналов ЭЭГ `hackathon_final.py`
