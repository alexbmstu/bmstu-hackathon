...
ser.open()
ser.isOpen()
# Infinite loop for parsing and storing data in FIFO
valuechannel1 = deque([])
valuechannel2 = deque([])
valuechannel3 = deque([])
counter = 0
...
# verification headererification header
 ...
 # print headerfirstbyte, " ",headersecondbyte, " ",headerthirtbyte 
 if headerfirstbyte == 1 and headersecondbyte == 28 and headerthirtbyte == 32:
  data_of_byte = data_of_byte + ser.read(520) 
  size = change2byte(data_of_byte[7:9])
  # print int(size[0])
  if int(size[0]) == 512:
   x = 9
   data_for_matrix = data_of_byte[0:521]
   crc = data_of_byte[521:523]
   matrix_of_value = [[0] * 16 for i in range(8)]
   for i in range(16):
    for j in range(8):
      value = change4byte(data_for_matrix[x:x+4])
      x += 4
      matrix_of_value[j][i] = int(value[0])
    #   print 'i = ', i, 'j = ', j
    #   print 'matrix_elem = ', matrix_of_value[j][i]
    # print "_________________________________"
   crc_check = calc(data_for_matrix)
   crc = ''.join([ crc[x:x+2][::-1] for x in range(0, len(crc), 2) ])
   crc = change2byteCRC(crc)
  
   if int(crc[0]) == crc_check:
     print int(crc[0])
     print crc_check 
     # print "ok"
     # print counter
     # print matrix_of_value[0:15][0]
     if counter <= 2:   #data in FiFo = 2
      ...
      counter += 1
      print list(valuechannel1)
      print "__________________"
      print list(valuechannel2)
      print "_________________"
      print list(valuechannel3)
     else:
      valuechannel1.append(matrix_of_value[0:15][0])
      valuechannel2.append(matrix_of_value[1:15][0])
      valuechannel3.append(matrix_of_value[2:15][0])
      valuechannel1.popleft()
      valuechannel2.popleft()
      valuechannel3.popleft()
      print list(valuechannel1)
      print "__________________"
      print list(valuechannel2)
      print "_________________"
