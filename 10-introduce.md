...
Данные от нескольких электродов передаются в вычислительный модуль ЭЭГ, где они преобразуются в числовые значения, фильруются, собираются в пакеты и передаются по интерфейсу RS232 в микрокомпьютер `RaspberryPi`.
